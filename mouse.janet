...
(defn to
  ...
  (evemu/event device "EV_ABS" "ABS_X" (if (zero? x) "1" "0") false)
  (evemu/wait-events device)
  (def x (->> x (* 10000) math/round string))
  (def y (->> y (* 10000) math/round string))
  (evemu/event device "EV_ABS" "ABS_X" x false)
  (evemu/event device "EV_ABS" "ABS_Y" y true))
  # (ev/sleep *delay*))
...
(defn click
  ...
  (evemu/event device "EV_KEY" keycode "1" true)
  (evemu/event device "EV_KEY" keycode "0" true)
  (ev/sleep *delay*))
...
(defn down
  ...
  [&opt button]
  (default button :left)
  (evemu/event device "EV_KEY" (button->keycode button) "1" true)
  (ev/sleep *delay*))
(defn up
  "Release the :left or :right mouse button."
  [&opt button]
  (default button :left)
  (evemu/event device "EV_KEY" (button->keycode button) "0" true)
  (ev/sleep *delay*))
(defn drag
